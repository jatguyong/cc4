...
public class BucketSort {
    ...
    public static void bucketSort(int[] arr) {
        int maxValue = getMaxValue(arr);counter++;
        int bucketSize = Math.min(10, maxValue / arr.length + 1);counter++;
        // Create buckets
        List<Integer>[] buckets = new ArrayList[(maxValue / bucketSize) + 1];counter++;
        counter++;
        for (int i = 0; i < buckets.length; i++) {counter++;counter++;
            buckets[i] = new ArrayList<>();counter++;
        }
        counter++;
        // Put elements into buckets
        for (int num : arr) {counter++;counter++;
            int bucketIndex = num / bucketSize;counter++;
            buckets[bucketIndex].add(num);counter++;
        }
        counter++;
        // Sort each bucket
        for (List<Integer> bucket : buckets) {counter++;counter++;
            Collections.sort(bucket);counter++;
        }
        ...
        for (List<Integer> bucket : buckets) {counter++;counter++;
            counter++;
            for (int num : bucket) {counter++;counter++;
                arr[index++] = num;counter++;
            }
        }
    }

    private static int getMaxValue(int[] arr) {
        int maxValue = Integer.MIN_VALUE;counter++;
        counter++;
        for (int num : arr) {counter++;counter++;
            maxValue = Math.max(maxValue, num);counter++;
        }
        counter++;
        return maxValue;
    }
